...
with st.sidebar:
    ...
    show_qin_overall_heat_table = st.toggle("儲存連贏圖", key="show_qin_overall_heat_table", value=True)
# --- 賽事資料加載 ---
    ...
    while monitoring_on:
        ...
        with placeholder.container():
            ...
            if not prediction_df.empty:
                high_flow_df = prediction_df[prediction_df['MoneyFlow'] > 300]
                ...
                with st.expander("🚨 異常大額資金流紀錄 (MoneyFlow > 300)", expanded=False):
                    if st.session_state.high_moneyflow_alerts.empty:
                        st.info("目前尚無大於 300 的資金流紀錄。")
                    else:
                        # 將最新紀錄排在最上面以利閱讀
            ...
            if show_qin_overall_heat_table:
                render_qin_overall_heat_table(
                    ...
                    max_minutes=10  # 自動拉取最近 15 分鐘的數據 (共 15 個欄位)
                )
